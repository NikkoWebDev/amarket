...
export default function FeedbackPanel({ feedback, idProyecto, userRole, onUpdate }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await api.post('/feedback', { id_proyecto: idProyecto, comentario, aprobado });
      setComentario('');
      setAprobado(false);
      onUpdate();
    } catch (err) {
      alert(err.response?.data?.error || 'Error al enviar feedback');
    }
  };

  const handleToggleAprobado = async (fb) => {
    try {
      await api.put(`/feedback/${fb.id_feedback}`, { aprobado: !fb.aprobado });
      onUpdate();
    } catch (err) {
      alert(err.response?.data?.error || 'Error al actualizar feedback');
    }
  };
