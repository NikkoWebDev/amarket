...
const NAV_ITEMS = [
  ...
  { id: "kanban",    label: "Proyectos",  Icon: Icon.Kanban,   badge: 3 },
  { id: "feedback",  label: "Feedback",   Icon: Icon.Chat,     badge: 1 },
  { id: "ia",        label: "IA Insights",Icon: Icon.AI },
  { id: "settings",  label: "Ajustes",    Icon: Icon.Settings },
];

export default function Navigation({ active, onChange }) {
  return (
    // Gradient fade-out so content doesn't hard-cut behind the nav
...
function NavButton({ item, isActive, onClick }) {
  const { label, Icon: ItemIcon, badge } = item;
  ...
  return (
    <button
      ...
      className={`
        ...
        ${isActive
          ? /* Active: floating pill with gradient */
      ...
      {badge && !isActive && (
        <span
          className="absolute top-1 right-1 sm:right-2
                     bg-petal-500 text-white text-[9px] font-bold
                     rounded-full min-w-[16px] h-4 px-[3px]
                     flex items-center justify-center
                     border-[1.5px] border-white
                     animate-badge-pop shadow-sm"
        >
          {badge}
        </span>
      )}
